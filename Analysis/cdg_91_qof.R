...
cat("INFO: q91: processign lookups ...", "\n")
...
qof.orgref <- qof.orgref %>%
    select(starts_with("practice")
           , starts_with("ccg"))
...
qof.ind <- qof.ind %>%
    ...
    merge(qof.orgref %>% select(practice_code, ccg_code)
          , by = "practice_code") %>%
    merge(qof.indmap %>% filter(is.register == FALSE) %>%
...
qof.ind.measures <- qof.ind.combined %>%
    filter(!is.na(value)) %>%
...
if (bWriteCSV) {
    ...
    fwrite(qof.ind.measures, "./Results/qof_ind__eng_ccg_prac__measure_ndv.csv")

} else {
    cat("INFO: cdg_91_qof: NOT saving qof.ind.combined ...", "\n")
}

qof.ind.measures %>% filter(FALSE) %>% str()
...
qof.prev.measures <- qof.prev.combined %>%
    filter(!is.na(value)) %>%
    # spin up numerator, denominator, exceptions
    dcast(... ~ measure, fun = sum, value.var = "value") %>%
    # cross join measures and combine and remove intermediate columns
...
if (bWriteCSV) {
    ...
    fwrite(qof.prev.measures, "./Results/qof_prev__eng_ccg_prac__measure_ndv.csv")
...
#
# COMPARE - Add England comparator and signficance test ####
#
...
if (!exists("qof.prev.measures"))
    qof.prev.measures <- fread("./Results/qof_prev__eng_ccg_prac__measure_ndv.csv")

qof.prev.measures$org.type %>% unique() %>% print()
...
qof.prev.var.cast <- qof.prev.measures %>%
...
qof.prev.ref.cast <- qof.prev.measures %>%
