...
sb__this_measure <- reactive(input$sb__this_measure)
...
sb__ind_dat <- reactive({
    ...
    l_ind_melt <- l_ind %>% setDT() %>%
        ...
        mutate_at(vars(status), factor, levels = fields_ordered) %>%
        mutate(pvalue = value / register) %>%
        mutate(sort_key = display_order + ifelse(display_order < 20, 0, pvalue)) %>%
        mutate(
            #practice_code_ordered = factor(practice_code)
            #, practice_code_ordered = reorder(practice_code_ordered, desc(practice_code))
             practice_code_ordered = reorder(factor(practice_code), desc(practice_code))
            #, practice_code_ordered = reorder(practice_code_ordered, desc(practice_code))
            , practice_code_ordered = reorder(practice_code_ordered, desc(sort_key))
        )
...
renderPlot({
    ...
    p <- ggplot(l_ind_melt) +
        ...
        geom_text(
            ...
            data = l_ind_melt %>% subset(value > 0 & status %in% c("optimal", "exceptions"))
            , aes(
                x = practice_code_ordered
                , y = pvalue
