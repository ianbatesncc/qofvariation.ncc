...
measures <- dat$measures #%>% l_add_orgtag()
compare <- dat$compare #%>% l_add_orgtag()
...
renderPlot({
  ...
  this_dat <- compare %>%
    ...
    filter(practice_code %in% tmp_practices) %>%
    mutate(
...
DT::renderDataTable({
  this_m <- compare %>% 
    ...
    select(-org.type.var, -data_source, -m.name, -m.type) %>%
    select(ccg_code, practice_code, indicator_group_code, indicator_code, org.type.ref, everything()) %>%
    dcast(... ~ compare.type + compare.param, fun = function(x){paste(x, collapse = "")})
