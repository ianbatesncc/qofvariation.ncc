...
sb__measures <- measures %>%
    ...
    select(ccg_code, practice_code, indicator_code, m.name, m.stat, value, measure_label)
...
sb__tmp_practices_list <- reactive({
    ...
    ccg_group__ccgs <- lu.orgs.ccgs.groups %>% filter(
...
sb__ind_dat <- reactive({
    ...
    l__sort_measure <- sb__measures %>%
        filter(
            practice_code %in% l__tmp_practices_list | 
                ((practice_code == "ccg") & (ccg_code %in% l__tmp_practices_list))
    ...
    l_ind <- counts %>%
        filter(
            ...
            l__sort_measure %>% select(-starts_with("m"))
            , by = c("ccg_code", "practice_code", "indicator_code")
            # reverse value if descending
        ) %>% mutate(
            value = (2 * l__sort_desc - 1) * value
        )
    
    # measures %>% filter(practice_code %in% l__tmp_practices_list) %>% filter(m.stat != "denominator") %>% dcast(... ~ m.name + m.stat)
    all_measure_stats <- measures %>% select(m.type, m.name, m.stat) %>% unique()
    ...
    practices_ordered <- l_ind %>%
        arrange(display_order, value) %>%
        .$practice_code
    ...
    l_ind_melt <- l_ind %>%
        ...
        mutate(
            practice_code_ordered = ordered(practice_code, levels = rev(practices_ordered))
        )
