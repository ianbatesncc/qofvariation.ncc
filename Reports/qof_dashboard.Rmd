...
this_ind_group <- reactive(input$this_ind_group)
this_practice <- reactive(input$this_practice)
this_measure <- reactive(input$this_measure)
...
DT::renderDataTable({
    this_m <- measures %>% 
        filter(
            practice_code == this_practice()
            , indicator_group_code == this_ind_group()
            , m.name == this_measure()
        ) %>%
