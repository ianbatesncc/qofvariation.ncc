...
hm__heatmap_dat__ggplot <- reactive({
    ...
    # for development -
    l__this_ind_group <- "[All]"
    l__this_measure <- "exceptions"
    l__this_comparator <- "england"
    l__this_comparison_method <- "spc"
    l__tmp_practices_list <- c("eng", "n2", "02Q", "C84001", "C84008", "C84013")
    l__hm__this_sort_option <- "practice_code"
    l__hm__sort_desc <- TRUE
    ...
    if (l__hm__this_sort_option != "summary_measure") {
        this_sort <- this_sort %>% mutate(value_sort = 0)
    }
    
    # sorting based on intermediate    
...
renderPlot({
    ...
    p <- ggplot(l_ind_melt) +
        ...
        #ylabel <- "Counts scaled relative to register (percent)"
    if (sb__scale_counts_largest_gp() == TRUE)
        p <- p + labs(y = "Counts scaled relative to largest GP register overall (percent)") 
