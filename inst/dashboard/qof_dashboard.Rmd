...
require("ggplot2")
#require("d3heatmap")
require("DT", warn.conflicts = FALSE)
...
qv <- qof__main__load(qof_root = c("qof-1718", "qof-1617", "qof-1516"))
...
l__filter_domain_codes_tokeep <- function(
    x, domain_codes = c("CL", "PH", "PHAS")
) {
    domain_codes <- match.arg(domain_codes)
...
# alter sense of exceptions for heatmap.
# for exceptions Change Worse -> Better and Better -> Worse.  All other measures
# to be left alone.

lu_exceptions_swap_sense <- fread(input = "
m.name,statsig,statsig_override
exceptions,Better,Worse
exceptions,Worse,Better
")

compare <- compare %>% setDT() %>%
    .[lu_exceptions_swap_sense, on = c("m.name", "statsig"), statsig := statsig_override]
...
these_qof_periods <- measures$qof_period %>% unique() %>% sort()
...
# for e.g. Heatmap
# converts a string to a value.  Hmmm.

lu_statsig_num__highisbetter <- fread(input = "
statsig,istatsig
Better,1
Worse,-1
"
...
lu_statsig_num__highisworse <- lu_statsig_num__highisbetter %>%
    mutate(istatsig = -istatsig)

lu_statsig_num__hilo <- fread(input = "
statsig,istatsig
Higher,1
Similar,0
Lower,-1
...
lu_statsig_num <- list(
    lu_statsig_num__hilo
    , lu_statsig_num__highisbetter
    #, lu_statsig_num__highisworse
) %>% bind_rows()
...
# TODO v: dependent on qof_period
...
counts <- measures %>%
    ...
    merge(
        register
        , by = c("qof_period", "ccg_code", "practice_code", "indicator_group_code")
        , all.x = TRUE
    ) %>%
    rename(
...
sb__measures__prevalence <- measures %>%
    # generate frame
    #filter(!indicator_code %in% lu__register_indicators) %>% 
    select(qof_period, ccg_code, practice_code, indicator_group_code, indicator_code) %>%
    unique() %>%
...
selectInput(
    "hm__this_qof_period"
    , label = "Select QOF period:"
    , choices = these_qof_periods
    , selected = these_qof_periods[1]
    , selectize = FALSE
    , multi = FALSE
)
...
hm__this_qof_period <- reactive(input$hm__this_qof_period)
...
hm__tmp_practices_list <- reactive({
    ...
    # For development
    l__this_qof_period <- these_qof_periods[1]
    l__this_ccg_group <- "Mid. Notts. UOP"
    l__bool_hide_practices <- FALSE
    
    l__this_qof_period <- hm__this_qof_period()
...
if (bRenderggplot) {
    ...
    renderPlot({
        ...
        p <- ggplot(
            ...
            labs(caption = "Source: Secondary analysis of NHS Digital QOF data")
...
sb__this_qof_period <- reactive(input$sb__this_qof_period)
...
sb__tmp_practices_list <- reactive({
    ...
    l__this_qof_period <- sb__this_qof_period()
...
sb__ind_dat <- reactive({
    ...
    l__this_ccg <- orgref %>%
        filter(
            qof_period == l__this_qof_period
            , practice_code == l__this_practice
        ) %>%
        .$ccg_code
    ...
    l_ind <- counts %>%
        filter(
            ...
            , by = c("qof_period", "ccg_code", "practice_code", "indicator_code")
